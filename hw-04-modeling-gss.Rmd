...
gss <- gss %>%
  mutate(
    ...
    advfront_recode = factor(advfront_recode, levels = c("Agree", "Not agree"))
  )

# Count the new levels
gss %>%
...
gss <- gss %>%
  mutate(
    polviews_recode = case_when(
      ...
      TRUE ~ NA_character_  # Keep NA or other undefined responses as NA
    ),
    polviews_recode = factor(polviews_recode, levels = c("Conservative", "Moderate", "Liberal"))
  )
...
gss16_fit_rs_1 <- gss16_wflow_1 %>%
  fit_resamples(resamples = gss16_folds, metrics = metric_set(roc_auc, accuracy))
